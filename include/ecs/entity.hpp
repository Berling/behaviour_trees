    ...
    public:
        ...
        component_type& emplace_back(arguments... args) {
            ...
            if (it == components_.end()) {
                components_.emplace(std::make_pair(component_type::type_id(), std::make_unique<component_type>(std::forward<arguments>...)));
            } else {
                utils::log(utils::log_level::LOG_WARNING) << "entity already has component with type id " << component_type::type_id() << std::endl;
            }
            return *components_[component_type::type_id()];
        }
